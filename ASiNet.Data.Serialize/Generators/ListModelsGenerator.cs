...
public class ListModelsGenerator : IModelsGenerator
{
    ...
    private Expression SerializeElements(ISerializerContext serializeContext, Expression count, Expression list, Expression model, Expression writer)
    {
        ...
        return Expression.Block([i],
            ...
            Expression.Loop(
                Expression.IfThenElse(
                    ...
                    Expression.Block(
                        ExpressionsHelper.CallSerialize(model, Expression.Property(list, "Item", i), writer, Expression.Constant(serializeContext)),
    ...
    private Expression DeserializeElements(ISerializerContext serializeContext, Expression count, Expression list, Expression model, Expression reader)
    {
        var i = Expression.Parameter(typeof(int), "i");
        var breakLabel = Expression.Label("LoopBreak");
        return Expression.Block([i],
            ...
            Expression.Loop(
                Expression.IfThenElse(
                    ...
                    // ADD AND DESERIALIZE ELEMENT
                    Expression.Block(
                        Expression.Call(list, nameof(List<byte>.Add), null, ExpressionsHelper.CallDeserialize(model, reader, Expression.Constant(serializeContext))),
                        Expression.AddAssign(i, Expression.Constant(1)))
                    ),
                breakLabel)
            );
    ...
    private Expression GetElementsSize(ISerializerContext serializeContext, Expression count, Expression list, Expression model, Expression result)
    {
        var i = Expression.Parameter(typeof(int), "i");
        var breakLabel = Expression.Label("LoopBreak");
        return Expression.Block([i],
            ...
            Expression.Loop(
                Expression.IfThenElse(
                    ...
                    // ADD AND DESERIALIZE ELEMENT
                    Expression.Block(
                        Expression.AddAssign(result, ExpressionsHelper.CallGetSizeGenerateTime(model, Expression.Property(list, "Item", i), Expression.Constant(serializeContext))),
                        Expression.AddAssign(i, Expression.Constant(1)))
                    ),
                breakLabel)
            );
